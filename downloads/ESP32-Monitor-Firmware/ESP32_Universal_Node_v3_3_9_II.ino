/* =============================================================================
 *  ESP32 Monitor v3.3.9 — dualbroadcast_safe (board: S2)
 
 *  Board-specific constants (unchanged from S2):
 *    DEFAULT_SOIL_PIN = 35
 *    DEFAULT_SDA_PIN  = 21
 *    DEFAULT_SCL_PIN  = 22
 *    DEFAULT_NAME     = "S2"
...
// Must be >= baseline DS_ADV_DURATION_MS (4 s).
#define SMART_ALERT_PRE_ADV_MS 4000

// DUAL-BROADCAST (v3.3.8/v3.3.9): each phase duration.
// 3 s per phase is empirically the minimum for Android MyAmbience and
// macOS bleak scanners to reliably catch one advertisement.
...
struct Config {
  ...
  // When true the plate takes runSmartDsWakePath. Set via BLE command EMAIL_EN:0|1.
...
// ─── v3.3.9: NimBLE init tracker ─────────────────────────────────────────────
// NimBLE-Arduino 1.x doesn't expose getInitialized(), so we track it ourselves.
// Set true in runColdBootPath() and broadcastStatusAdv()/broadcastSensirionAdv().
// Checked in enterDeepSleep() so we only run teardown when there's something to tear down.
...
// ─── Deep Sleep entry ────────────────────────────────────────────────────────
// v3.3.9: CLEAN radio teardown before esp_deep_sleep_start().
// Teardown order:
//   1. Close logFile (flush LittleFS journal)
//   2. Disconnect BLE clients politely
//   3. NimBLEDevice::deinit(true) — releases BT controller
//   4. WiFi teardown
//   5. esp_bt_controller_disable/deinit — releases analog block
//   6. 300ms settle
//   7. Lower CPU, then esp_deep_sleep_start()
void enterDeepSleep(uint32_t seconds) {
  char buf[96];
  ...
  if (g_nimbleInited) {
    ...
    // v3.3.9: extended delays — gives BT stack more time to fully release.
  ...
  // WiFi teardown (no-op if WiFi was never started).
  ...
  // Fully disable BT controller — releases the analog block.
  // Safe even if controller was never enabled — both calls are idempotent.
  esp_bt_controller_disable();
  esp_bt_controller_deinit();

  // v3.3.9: extended to 300 ms — v3.3.8 lab logs showed brownout reset
  // with 100 ms after dual-broadcast workflow.
  delay(300);
...
// ─── Sensirion Gadget BLE broadcast (v3.3.9 dual-broadcast Phase A) ──────────
// Initializes Sensirion DataProvider (sets up NimBLE + 0x06D5 mfg-data adv),
// writes current sensor values, commits sample, broadcasts for durMs.
// After the wait, stops advertising so caller can swap in 0xFFFF payload.
void broadcastSensirionAdv(float t, float h, float s, uint32_t durMs) {
  // v3.3.9 BROWNOUT FIX: lower CPU during heavy BLE stack init.
  ...
  // v3.3.9: settling gap before next phase.
...
void handleCommand(const String& cmd) {
  ...
  // SMART-WAKE: enable/disable feature gate.
...
// Stage 1: measure + CSV + dual-broadcast (Sensirion + 0xFFFF).
// Stage 2 (only when checkAlertsPending=true): WiFi+email, wifiOff.
...
void setup() {
  ...
  Serial.println("\n╔══════════════════════════════════════════════════════════════════════╗");
  Serial.println("║   ESP32 Monitor v3.3.9_dualbroadcast_safe (S2) — SMART-WAKE         ║");
  ...
  if (wokeFromDeepSleep) {
    ...
    //                        Stage 2 Wi-Fi/SMTP only on alert AND cooldown).
    if (!cfg.emailEnabled) {
      plateState = STATE_DS_WAKE;
      runDsWakePath();
  ...
  Serial.println("\n╔══════════════════════════════════════════════════════════════════════╗");
  Serial.println("║                  READY v3.3.9 (S2)                                  ║");
  Serial.println("╚══════════════════════════════════════════════════════════════════════╝\n");
}
